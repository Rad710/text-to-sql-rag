...
## Done when
- [ ] `Settings` exposes `llm_base_url` (+ `llm_model`, `llm_api_key`); `OpenAIProvider` uses them.
- [ ] `.env.example` + README document the Ollama and vLLM setups; mock remains the default with no key.
- [ ] Unit test covers the `base_url`/model wiring; `ruff`/`mypy`/`pytest` green.
- [ ] Committed.
