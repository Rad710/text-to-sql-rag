...
test("mounts without crashing and shows the shell", async () => {
  ...
  expect(screen.getByText(/DYR Transportes — Data Assistant/)).toBeInTheDocument();
  // The empty-thread welcome + suggestions render after the runtime settles (effect).
  expect(await screen.findByRole("heading", { name: /qué querés saber/i })).toBeInTheDocument();
  expect(screen.getByRole("button", { name: /facturación total por ruta/i })).toBeInTheDocument();
